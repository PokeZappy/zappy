...
namespace Zappy {
    void Raylib::drawMenu() {
        ...
        if (elapsed_seconds.count() < blinkTime) {
            raylib::Color(0, 0, 0, (255 / blinkTime) * (elapsed_seconds.count())).DrawText("Appuyez sur ENTREE pour continuer . . .", 200, 900, 75);
        } else if (elapsed_seconds.count() >= blinkTime && elapsed_seconds.count() <= blinkTime * 2) {
            raylib::Color(0, 0, 0, 255 - (255 / blinkTime) * (elapsed_seconds.count() - blinkTime)).DrawText("Appuyez sur ENTREE pour continuer . . .", 200, 900, 75);
        } else 
