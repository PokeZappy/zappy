...
// Defines and Macros
//----------------------------------------------------------------------------------
// LOCAL PATCH: tuned for the _gridSize = 0.01 world. raylib 6.0 made camera speed
// frame-rate independent (units per second), so the old 0.001f per frame becomes
// 0.001 * 60 = 0.06f here.
#define CAMERA_MOVE_SPEED                           0.06f      // Units per second
...
void UpdateCamera(Camera *camera, int mode)
{
    ...
    else
    {
        ...
        else if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT))  // LOCAL PATCH: only rotate while right-dragging
        {
            // Mouse support
    ...
    if ((mode == CAMERA_THIRD_PERSON) || (mode == CAMERA_ORBITAL) || (mode == CAMERA_FREE))
    {
        ...
        // LOCAL PATCH: gated on CTRL so the wheel stays free for the HUD scroll
        if (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL)) CameraMoveToTarget(camera, -GetMouseWheelMove());
