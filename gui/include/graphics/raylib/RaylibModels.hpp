...
namespace Zappy {
    ...
    class RaylibModels {
        public: 
            RaylibModels(std::string id, raylib::Shader &shader) {
                std::string shinyPath = "assets/textures/pokemons/" + id + "_shiny.png";
                ...
                if (indexIdle != -1) {
                    _models[Animations::IDLE] = std::make_shared<RaylibModel>(id, shader, _animations[indexIdle]);
                }
                if (indexWalk != -1) {
                    _models[Animations::WALK] = std::make_shared<RaylibModel>(id, shader, _animations[indexWalk]);
                }
                if (indexSleep != -1) {
                ...
                // _models[Animations::NONE] = std::make_shared<RaylibModel>(id, shader, _animations[indexCry]);
